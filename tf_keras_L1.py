...
def batch_distance_matrix_general(A, B):
    """ This calculates the L-1 norm between two points"""
    with tf.name_scope('dmat'):
        r_A = tf.reduce_sum(A , axis=2, keepdims=True)
        r_B = tf.reduce_sum(B , axis=2, keepdims=True)
        D = tf.abs(r_A - tf.transpose(r_B, perm=(0, 2, 1)))
        return D
...
def get_particle_net_L1(num_classes, input_shapes):
    ...
    return keras.Model(inputs=[points, features, mask], outputs=outputs, name='ParticleNet_L1')


def get_particle_net_lite_L1(num_classes, input_shapes):
    ...
    return keras.Model(inputs=[points, features, mask], outputs=outputs, name='ParticleNet_Lite_L1')
